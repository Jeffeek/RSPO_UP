...
namespace RSPO_UP_1.Algorithms
{
	public class SortingAlgo : IAlgo
	{
		...
		public SortingAlgo() => _movies = new MovieDeserializer().GetMovies();

		public SortingAlgo(List<Movie> list) => _movies = list;
		...
		{
			foreach (var sortedItem in list) Console.WriteLine(sortedItem);
		}

		public List<Movie> SortByName()
		{
			var sortedItems = _movies.OrderBy(x => x.Name).ToArray();
		...
		{
			var sortedItems = _movies.OrderBy(x => x.Price).ToArray();
		...
		{
			var sortedItems = _movies.OrderBy(x => x.Date).ToArray();
		...
		{
			var sortedItems = _movies.OrderBy(x => x.CinemaName).ToArray();
