...
namespace RSPO_UP_5.ViewModel
{
	public class MatrixWindowViewModel : ViewModelBase
	{
		...
		{
			FillFirstMatrixRandomlyCommand =
				new RelayCommand(FillFirstMatrixRandomlyExecuted, CanFillFirstMatrixExecute);

			FillFirstMatrixByHandCommand = new RelayCommand(FillFirstMatrixByHandExecuted, CanFillFirstMatrixExecute);
			FillSecondMatrixRandomlyCommand =
				new RelayCommand(FillSecondMatrixRandomlyExecuted, CanFillSecondMatrixExecute);

			FillSecondMatrixByHandCommand =
				new RelayCommand(FillSecondMatrixByHandExecuted, CanFillSecondMatrixExecute);
			...
			MultiplyFirstAndSecondMatricesCommand =
				new RelayCommand(MultiplyFirstAndSecondMatricesExecuted, CanMultiplyMatrices);
		...
		private bool CanReverseFirstMatrixExecute() =>
			FirstMatrix?.Columns.Count == FirstMatrix?.Rows.Count && _firstMatrixColumnsCount > 1;
		...
		private bool CanReverseSecondMatrixExecute() =>
			SecondMatrix?.Columns.Count == SecondMatrix?.Rows.Count && _secondMatrixColumnsCount > 1;
