...
namespace RSPO_UP_5.Model
{
	public class Matrix : IEquatable<Matrix>, IDisposable
	{
		...
		public void Dispose()
		{
			if (!_isDisposed) GC.SuppressFinalize(this);
		}
		...
		private Matrix InternalMulAtoB(Matrix b)
		{
			...
			for (var i = 0; i < _innerMatrix.GetLength(0); i++)
			{
				...
				{
					for (var k = 0; k < b.RowsCount; k++) matrix[i, j] += Math.Round(_innerMatrix[i, k] * b[k, j], 2);
				}
			}
		...
		private Matrix InternalMulBtoA(Matrix b)
		{
			...
			for (var i = 0; i < _innerMatrix.GetLength(0); i++)
			{
				...
				{
					for (var k = 0; k < _innerMatrix.GetLength(0); k++)
		...
		{
			var w = _innerMatrix.GetLength(0);
			var h = _innerMatrix.GetLength(1);

			var result = new Matrix(h, w);

			for (var i = 0; i < w; i++)
			{
				for (var j = 0; j < h; j++) result[j, i] = _innerMatrix[i, j];
			}
		...
		public void ProcessFunctionOverData(Action<int, int> func)
		{
			...
			{
				for (var j = 0; j < ColumnsCount; j++) func?.Invoke(i, j);
			}
		}
		...
		{
			if (columnIndex < 0 ||
			    columnIndex >= ColumnsCount) throw new ArgumentException("invalid column index");
		...
		{
			if (rowIndex < 0 ||
			    rowIndex >= RowsCount) throw new ArgumentException("invalid row index");
		...
		public double CalculateDeterminant()
		{
			...
			if (ColumnsCount == 2) return Math.Round(this[0, 0] * this[1, 1] - this[0, 1] * this[1, 0], 2);
			double result = 0;
			for (var j = 0; j < ColumnsCount; j++)
			{
				result += Math.Round((j % 2 == 1 ? 1 : -1) *
				                     this[1, j] *
		...
		public Matrix CreateInvertibleMatrix()
		{
			...
			ProcessFunctionOverData((i, j) => result[i, j] = Math.Round((i + j) % 2 == 1
				                                                            ? -1
				                                                            : 1 *
				                                                              CalculateMinor(i, j) /
				                                                              determinant, 2));
		...
		private double CalculateMinor(int i, int j) => Math.Round(CreateMatrixWithoutColumn(j)
